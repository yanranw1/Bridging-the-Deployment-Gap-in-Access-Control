...
#
# FLAN-T5 is an instruction-tuned seq2seq model. It was trained on flat,
# natural-language instructions -- NOT on chat-role markup, not on stringified
# Python message lists, and not on long "output ONLY JSON, no markdown" system
# prompts. Feeding it str([{"role": "system", ...}]) (the old behaviour) buries
# the actual task inside Python/JSON punctuation it has no reason to follow.
#
# This builder produces a single flat instruction string: a short task line,
# a compact action list, the flattened conversation, and one clear output
# directive at the very end (T5 attends well to a trailing instruction).
# --------------------------------------------------------------------------- #

# Compact one-line-per-action list. The verbose signature catalogue above is
# fine for GPT-4o but too noisy for flan-t5-base, so we hand it a terse menu.
_FLAN_ACTIONS = (
    "list_emails, get_email, search_emails(query), analyze_email(email_id), "
    "draft_reply(email_id), send_email(to,subject,body), send_draft(draft_id), "
    "draft_email(to,subject,body), forward_email(email_id,to), "
    "delete_email(email_id), star_email(email_id), summarize_inbox, "
    "create_task(title), list_tasks, complete_task(task_id)"
)


def _flatten_conversation(nl: list[dict]) -> str:
    parts = []
    for turn in nl:
        role = turn.get("role", "User")
        ...
        if "resource" in turn:
            parts.append(f"({role} sees: {json.dumps(turn['resource'])})")
    return " ".join(parts)


def build_flan_prompt(nl: list[dict]) -> str:
    """Flat instruction prompt suited to FLAN-T5's instruction-tuned style."""
    conversation = _flatten_conversation(nl)
    return (
        "Plan the email-agent actions for the conversation below. "
        "Output a JSON list of steps; each step has the keys "
        '"acp_id", "api", "args", "decision" (allow or deny), and "reason". '
        "Set decision to deny if the action would leak confidential or personal "
        "information or send something without needed approval. "
        "If no email action is needed, output []. "
        f"Allowed api names: {_FLAN_ACTIONS}. "
...
_FLAN_MODEL_NAME = "google/flan-t5-base"
_FLAN_TOKENIZER = None
_FLAN_MODEL = None


def _load_flan():
    """Load tokenizer + model once and reuse across records."""
...
def generate_code(nl: list[dict]) -> list[dict]:
    tokenizer, model = _load_flan()

    prompt = build_flan_prompt(nl)
    inputs = tokenizer(
        prompt,
        return_tensors="pt",
        truncation=True,
